...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#f2f2f2] pb-24 md:pb-0 font-['Plus_Jakarta_Sans'] selection:bg-blue-100 selection:text-blue-900 relative overflow-hidden">
              ...
              <div className="flex flex-col items-center text-center md:-ml-36">
                ...
                {activeSection === AppSection.SOLICITUD && hasSolicitudFilters && solicitudStep === 'crear' && (
                  <div className="flex gap-4 mb-4">
                    <button onClick={loadPreviousOrder} className="group relative overflow-hidden text-blue-600 font-black text-[10px] uppercase tracking-widest bg-white px-6 py-3 rounded-xl transition-all hover:bg-slate-50 active:scale-95 border border-slate-200 flex items-center gap-2 shadow-md"><ShineEffect /><div className="relative z-10 flex items-center gap-2"><ICONS.Delivery /> PEDIDO ANTERIOR</div></button>
                    <button onClick={() => setSolicitudFilters({ departamento: '', seccion: '' })} className="group relative overflow-hidden text-rose-500 font-black text-[10px] uppercase hover:underline tracking-widest bg-white px-6 py-3 rounded-xl transition-all hover:bg-slate-50 active:scale-95 border border-slate-200 flex flex-col items-center justify-center leading-none shadow-md"><ShineEffect /><div className="relative z-10 flex flex-col items-center"><span>VOLVER</span><span className="text-[7px] mt-0.5 opacity-60">FILTROS</span></div></button>
                  </div>
                )}
              </div>
            </div>
            <div className="md:hidden flex items-center gap-2"><CustomLogo trigger={activeSection} /></div>
          </div>
        </header>
            ...
            {solicitudStep === 'crear' ? (
              <>
                {!hasSolicitudFilters ? (
                  ...
                  <div className="space-y-4 animate-fade-in">
                    <div className="bg-white p-4 rounded-[28px] border border-slate-200 shadow-sm"><h4 className="text-[9px] font-black text-slate-800 uppercase mb-3 tracking-[0.2em] opacity-60">Añadir Productos</h4><AutocompleteSearch products={inventory} onSelect={addItemToOrder} placeholder="Escribe el nombre del producto..." /></div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-transparent px-2 py-4 flex justify-around items-center z-50">
        {navItems.map((item) => (
